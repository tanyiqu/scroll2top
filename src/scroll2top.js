...
window.onload = () => {
    ...
    if (script.attributes.background.value) {
        scroll2top_btn.style.backgroundColor = script.attributes.background.value;
    }
    ...
    scrollToTop() {
        ...
        scroll2top_timer = setInterval(function () {
            ...
            if (document.documentElement.scrollTop <= 0) {
                clearInterval(scroll2top_timer);
                // 再给按钮加上监听
                this.btn.addEventListener('click', this.scrollToTop, false);
            }
        }, 10);
    }
